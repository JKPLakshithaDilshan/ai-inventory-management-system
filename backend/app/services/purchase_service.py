...
class PurchaseService(BaseService[Purchase, PurchaseCreate, PurchaseUpdate]):
    ...
    async def update(
        self, db_obj: Purchase, obj_in: PurchaseUpdate
    ) -> Purchase:
    ...
    async def receive_purchase(
        self, purchase_id: int, items: list[dict], received_date: date
    ) -> Optional[Purchase]:
        """
        ...
        for item_data in items:
            ...
            purchase_item = next(
                (item for item in purchase.items if item.id == item_id), None
            )
        ...
        all_received = all(
            item.received_quantity >= item.quantity for item in purchase.items
        )

        purchase.received_date = received_date
        purchase.status = (
            PurchaseStatus.RECEIVED if all_received else PurchaseStatus.PENDING
        )
