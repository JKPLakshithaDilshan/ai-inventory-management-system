...
def log_database_config() -> None:
    ...
    2. Test with psql: /Applications/PostgreSQL\\ 16/bin/psql -U
        postgres -d ai_inventory_db
    ...
    logger.info(
        f"Masked URL:    {mask_password_in_url(str(settings.DATABASE_URI))}"
    )
    logger.info(
        f"SQLAlchemy:    {mask_password_in_url(str(settings.DATABASE_URI))}"
    )
