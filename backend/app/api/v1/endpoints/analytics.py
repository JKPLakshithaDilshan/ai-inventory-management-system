...
async def get_reorder_suggestions(
    days_lookback: int = Query(
        30, ge=1, le=365, description="Days to analyze for sales velocity"
    ),
    safety_stock_multiplier: float = Query(
        1.5, ge=1.0, le=5.0, description="Safety stock multiplier"
    ),
    min_lead_time_days: int = Query(
        7, ge=1, le=90, description="Minimum lead time in days"
    ),
    ...
    Returns products that need reordering with suggested quantities and
    urgency levels.
    """
    ...
    try:
        ...
        # Count by urgency
        critical_count = sum(
            1 for s in suggestions if s["urgency"] == "critical"
        )
        high_priority_count = sum(
            1 for s in suggestions if s["urgency"] in ["critical", "high"]
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error generating reorder suggestions: {str(e)}",
        )


@router.get(
    "/demand-forecast/{product_id}", response_model=DemandForecastResponse
)
async def get_demand_forecast(
    product_id: int,
    days_history: int = Query(
        30, ge=7, le=365, description="Days of historical data to analyze"
    ),
    days_forecast: int = Query(
        14, ge=1, le=90, description="Days to forecast into the future"
    ),
    method: str = Query(
        "weighted_average", regex="^(weighted_average|moving_average)$"
    ),
    db: AsyncSession = Depends(get_db),
...
async def detect_slow_moving_stock(
    days_lookback: int = Query(
        90, ge=30, le=365, description="Days to analyze for movement"
    ),
    min_days_no_sales: int = Query(
        30, ge=7, le=180, description="Minimum days without sales to flag"
    ),
    turnover_threshold: float = Query(
        0.5, ge=0.0, le=5.0, description="Minimum turnover ratio threshold"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(check_permission("ai:reorder:view")),
):
    ...
    Returns products with recommendations for action (clearance,
    promotion, etc.).
    """
    analytics_service = AnalyticsService(db)
    ...
    try:
        ...
        dead_stock_count = sum(
            1 for item in slow_movers if item["severity"] == "dead_stock"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error detecting slow-moving stock: {str(e)}",
        )
